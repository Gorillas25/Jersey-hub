...
// --- NOVO MODAL DE ONBOARDING (OBRIGATÓRIO) ---
...
export function UserCatalog() {
  ...
  useEffect(() => {
    // Se o perfil já foi carregado, não é um admin e não tem telefone, força o modal.
  ...
  // --- FUNÇÃO DE GERAR LINK SIMPLIFICADA (COMO ERA ANTES) ---
    ...
    <div className="min-h-screen bg-slate-50">
      {showOnboardingModal && (
        <OnboardingPhoneModal 
          onSaveSuccess={async () => {
            await refreshProfile(); // Puxa o perfil atualizado do banco
      ...
      <div className={`container mx-auto px-4 py-8 ${showOnboardingModal ? 'blur-sm pointer-events-none' : ''}`}>
        ...
        <div className="mb-6 flex flex-col md:flex-row gap-4">
          <div className="flex-1 relative">
            ...
            <input type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="Buscar por time ou título..." className="w-full pl-10 pr-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent" />
          </div>
        ...
        {filteredJerseys.length === 0 && <div className="text-center py-16"><p className="text-slate-500 text-lg">Nenhuma camisa encontrada</p></div>}
